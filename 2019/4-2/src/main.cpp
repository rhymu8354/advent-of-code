...
bool IsPossiblePassword(
    ...
    size_t sameCounter = 1;
    for (size_t i = 0; i < 5; ++i) {
        // Count the number of same digits in a group.
        if (digits[i] == digits[i+1]) {
            ++sameCounter;
        } else {
            // We reach here if we see a break between groups.
            // Check if the last group was a duo.  If so,
            // we found "qualified neighbors".
            if (sameCounter == 2) {
                foundSameNeighbors = true;
            }
            sameCounter = 1;

            // Check the rule saying each digit may not be less
    ...
    // We reach here at the end of the number, and we may have
    // been tracking a group of same digits, so check one more time
    // for a "qualified neighbors" group.
    if (sameCounter == 2) {
        foundSameNeighbors = true;
    }
