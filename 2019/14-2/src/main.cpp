...
size_t CalculateOreNeeded(
    size_t fuel,
    const std::unordered_map< std::string, Recipe >& recipes
) {
    std::unordered_map< std::string, size_t > quantitiesNeeded;
    std::unordered_map< std::string, size_t > inventory;
    quantitiesNeeded["FUEL"] = fuel;
    ...
    printf("%zu ORE is used to produce %zu FUEL\n", quantitiesNeeded["ORE"], fuel);
    printf("Left-over inventory:\n");
    for (const auto& inventoryEntry: inventory) {
        Chemical chemical;
        chemical.name = inventoryEntry.first;
        chemical.quantity = inventoryEntry.second;
        printf("   %s\n", FormatChemical(chemical).c_str());
    }
...
int main(int argc, char* argv[]) {
    ...
    auto maxOrePerFuel = CalculateOreNeeded(1, recipes);
    constexpr size_t ore = 1000000000000;
    auto minFuelProduced = ore / maxOrePerFuel;
    auto fuelProduced = minFuelProduced;
    auto oreNeeded = CalculateOreNeeded(fuelProduced, recipes);
    while (oreNeeded < ore) {
        fuelProduced *= 2;
        oreNeeded = CalculateOreNeeded(fuelProduced, recipes);
    }
    auto maxFuelProduced = fuelProduced;
    while (
        (oreNeeded != ore)
        && (maxFuelProduced - minFuelProduced > 1)
    ) {
        printf("min: %zu, max: %zu\n", minFuelProduced, maxFuelProduced);
        if (oreNeeded > ore) {
            maxFuelProduced = fuelProduced;
        } else {
            minFuelProduced = fuelProduced;
        }
        fuelProduced = (minFuelProduced + maxFuelProduced) / 2;
        oreNeeded = CalculateOreNeeded(fuelProduced, recipes);
    }
    return EXIT_SUCCESS;
}
